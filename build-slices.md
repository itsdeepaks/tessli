...
- V3.2 completed the AccessRoute contract pilot, V3.3 completed the Motion source-guide proof, V3.4 focused canonical Browse, V3.5 completed the homepage task entry, and V3.6 completed resource-card consistency; V3.7–V3.17 are the remaining ordered delivery sequence.
...
| V3.7  | Deterministic task retrieval      | NEXT     | V3.2                    |
...
V3.0 replaced this continuation boundary. The next repository slice is **V3.7 Deterministic task retrieval**. Verification remains a maintenance and evidence concern, not the next public product milestone. Do not invent provider checks, reuse stale profile fingerprints, store credentials or private provider content, or overstate availability.
